...
**Domain Structure:**
- **CloudPanel Admin:** cp.dipteshdey.in (access server management)
- **Application:** pydetective.dipteshdey.in (main website)
...
1. Access CloudPanel at https://cp.dipteshdey.in (after DNS setup)
...
**Important:** Before accessing CloudPanel, ensure DNS for cp.dipteshdey.in is configured in Cloudflare (see next section).

## Cloudflare DNS Configuration

In Cloudflare dashboard for dipteshdey.in:

**CloudPanel DNS Records (cp.dipteshdey.in):**
```
Type: A
Name: cp
...
TTL: Auto
Comment: CloudPanel admin access - MUST be DNS Only
```

**Application DNS Records (pydetective.dipteshdey.in):**
```
Type: A
Name: pydetective
IPv4 address: YOUR_VPS_IP_ADDRESS
...
TTL: Auto
Comment: Main application domain - proxied for CDN protection

Type: A
Name: www.pydetective
IPv4 address: YOUR_VPS_IP_ADDRESS
Proxy status: Proxied (orange cloud)
TTL: Auto
...
Type: CNAME
Name: pydetective
Target: pydetective.dipteshdey.in
Proxy status: Proxied (orange cloud)
TTL: Auto
Comment: Alternative access method
...
**Critical Notes:**
1. **cp.dipteshdey.in MUST be DNS Only (grey cloud)** - CloudPanel needs direct IP access
2. **pydetective.dipteshdey.in SHOULD be Proxied (orange cloud)** - For CDN and DDoS protection
3. Wait 5-10 minutes for DNS propagation before accessing cp.dipteshdey.in

## Cloudflare SSL/TLS Configuration

**Step 1: Configure SSL Mode for Both Domains**
...
**Step 2: Generate Origin Certificate for Application**
...
**Step 3: Generate Origin Certificate for CloudPanel (Optional)**
1. Go to SSL/TLS -> Origin Server -> Create Certificate
2. Configure:
   - Hostnames: *.cp.dipteshdey.in, cp.dipteshdey.in
   - Certificate Type: RSA (2048)
   - Validity: 15 years (maximum)
3. Download: certificate.pem and private_key.pem

**Step 4: Install Certificate on Server for Application**
```bash
...
# Upload certificate files (using nano to paste content directly)
...
**Step 5: Install Certificate on Server for CloudPanel**
```bash
# Create SSL directory for CloudPanel
sudo mkdir -p /etc/cloudpanel/ssl/cp.dipteshdey.in
sudo chown root:root /etc/cloudpanel/ssl/cp.dipteshdey.in
sudo chmod 755 /etc/cloudpanel/ssl/cp.dipteshdey.in

# Upload CloudPanel certificate files
sudo nano /etc/cloudpanel/ssl/cp.dipteshdey.in/certificate.pem
# Paste CloudPanel certificate content

sudo nano /etc/cloudpanel/ssl/cp.dipteshdey.in/private_key.pem
# Paste CloudPanel private key content

# Set proper permissions
sudo chmod 644 /etc/cloudpanel/ssl/cp.dipteshdey.in/certificate.pem
sudo chmod 600 /etc/cloudpanel/ssl/cp.dipteshdey.in/private_key.pem
```

**Step 6: Configure CloudPanel SSL for Application**
...
```bash
# Test SSL certificate for application
sudo nginx -t
sudo systemctl reload nginx

# Check SSL details for application
openssl s_client -connect pydetective.dipteshdey.in:443 -servername pydetective.dipteshdey.in

# Test SSL for CloudPanel
openssl s_client -connect cp.dipteshdey.in:443 -servername cp.dipteshdey.in

# Cloudflare diagnostic
curl -I https://pydetective.dipteshdey.in
curl -I https://cp.dipteshdey.in
```

## Website Deployment

**Step 1: Create Site in CloudPanel**
1. Login to CloudPanel at https://cp.dipteshdey.in
...
**CloudPanel Access Issues (cp.dipteshdey.in)**

Symptoms: Cannot access CloudPanel admin at cp.dipteshdey.in
Root Causes:
1. DNS not configured (grey cloud required)
2. SSL certificate missing for cp.dipteshdey.in
3. Firewall blocking port 8443
4. CloudPanel service not running

Solutions:
```bash
# Check DNS propagation
dig cp.dipteshdey.in

# Verify DNS is DNS Only (grey cloud)
# Check Cloudflare dashboard

# Test direct IP access (temporarily)
curl -k https://YOUR_SERVER_IP:8443

# Check firewall rules
sudo ufw status

# Verify CloudPanel service
sudo systemctl status cloudpanel
sudo systemctl restart cloudpanel

# Check Nginx for CloudPanel
sudo nginx -t
sudo systemctl reload nginx
```
...
- [ ] SSL/TLS certificates properly installed for both domains (Full Strict)
...
- [ ] cp.dipteshdey.in DNS is DNS Only (grey cloud)
...
systemctl is-active cloudpanel >> $LOG_FILE
```
...
- **CloudPanel Admin:** https://cp.dipteshdey.in (DNS Only - grey cloud)
- **Application:** https://pydetective.dipteshdey.in (Proxied - orange cloud)
